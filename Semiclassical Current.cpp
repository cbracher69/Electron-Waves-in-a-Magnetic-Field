/*****************************************************************************************
  ...
  Semiclassical Current Density (z-direction) Map in a Purely Magnetic Field
...
/* The output file name */
  	string BitmapFile = "At 50 (uniform current).bmp";
...
/* *** QUANTUM TOTAL CURRENT *** */

/* *** Total Current in Magnetic Field (dimensionless, in units of the free-particle current (m * k) / (Pi * hbar^3)) *** */
	double MagneticTotalCurrent(double Epsilon)
	{
	/* Sum Over Inverse Wave Numbers */
		double Sum = 0;

	/* Energy of First Landau Level */
		double Energy = Epsilon - 1.0;

	/* Sum Over Open Channels */
		while (Energy > 0)
		{
		/* Wave Number in Field Direction */
	...
	/* Current in Units of Free-Particle Current (me * k / (Pi * hbar^3) */
		Sum = Sum / sqrt(Epsilon);
...
/* Determine Pixel Color from Numerical Value [-1 ... 1] */
...
{
	/* Opening Message */
	cerr << "SEMICLASSICAL CURRENT.c++ - Semiclassical Current in a Homogeneous Magnetic Field\n"
...
/* Reserve Field for Density Information */
	double *CurrentDensity = new double[xSamples * zSamples];
...
/* Loop Through z Values */
	...
	/* Calculate Green Function As A Sum */
		...
		/* Uniform Approximation */
			...
			for (j = kMin; j <= kMax; ++j)
			{
				...
				/* Phase of the Wave Function */
					PhaseFast = ReducedAction(rSquare, z, Epsilon, tFast);
					Phase = PhaseFast - 0.5 * Pi * Maslov;

				/* Amplitude of the Wave Function */
					double rho = Density(rSquare, z, Epsilon, tFast);
					AmplitudeFast = sqrt(rho);

				/* Classical Particle Velocity in z Direction */
					double zVelFast = z / tFast;
				...
				/* Phase of the Wave Function */
					PhaseSlow = ReducedAction(rSquare, z, Epsilon, tSlow);
					Phase = PhaseSlow - 0.5 * Pi * Maslov;

				/* Amplitude of the Wave Function */
					rho = Density(rSquare, z, Epsilon, tSlow);
					AmplitudeSlow = sqrt(rho);

				/* Classical Particle Velocity in z Direction */
					double zVelSlow = z / tSlow;
				...
				/* Phase of the Wave Function */
					CAux = ComplexReducedAction(rSquare, z, Epsilon, ComplexT);
				...
				/* Complex Velocity in z Direction */
					complex<double> zVelComplex = z / ComplexT;
		...
		/* Calculate Desired Result(s) */
			...
			{
				Aux = ClassicalCurrentDensity;
			}
			else
			{
				if (ReturnSemiclassical == true)
				{
					Aux = real(conj(WaveFunction) * WaveFunctionPrime);
				}
				else
				{
					if (ReturnUniform == true)
					{
						Aux = imag(conj(UniformWaveFunction) * UniformWaveFunctionPrime);
					}
				}
			}

		/* Add Contribution To Total Current */

// Test Correction eps * eps / pi  (area factor, different "units" for density, total current)
			TotalCurrent += x * Aux * (xMax - xMin) / (xSamples - 1);

		/* Integrated Current Density, If Desired */
		...
		/* Store Density of Wave Function */
			CurrentDensity[i * xSamples + jj] = Aux;

		/* Find Average, Maximum Current Density */
			DensitySum += fabs(Aux);
			if (Aux > DensityMax) DensityMax = Aux;
			if (Aux < DensityMin) DensityMin = Aux;
		}

	/* Show Progress */
		cerr << "z = " << z << " (Total Current: " << 2.0 * fabs(TotalCurrent) << ")\n";

	/* Error Analysis For Total Current */
		CurrentSum += fabs(TotalCurrent);
...
/* Notify User */
	...
	cerr << "(Energy in hbar * omega: " << Epsilon << ")\n\n";
	cerr << "(Average Current Density: " << DensitySum / (xSamples * zSamples) << ")\n";
	cerr << "(Maximum Current Density: " << DensityMax << ")\n";
	cerr << "(Minimum Current Density: " << DensityMin << ")\n\n";
	cerr << "(Theoretical Total Current: " << MagneticTotalCurrent(Epsilon) << ")\n";
	cerr << "(Relative Current Deviation: " << 1.0 - CurrentSum / (zSamples * MagneticTotalCurrent(Epsilon)) << ")\n\n";
...
/* Loop Over z Values */
	...
	/* Loop Over x Values */
		...
		/* Calculate Relative Density */
			Aux = CurrentDensity[i * xSamples + j] / MaxCurrent;

		/* Apply Saturation Offset */
			Aux *= SaturationOffset;
			if (fabs(Aux) > 1.0) Aux = signum(Aux);
		...
		/* Sort Out Unreliable Results */
			...
			/* Assign Coloring */
				AssignColor(&Red, &Green, &Blue, signum(Aux) * Saturation);
			}
