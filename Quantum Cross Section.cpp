...
/* Version Info */
	...
	const char *VersionDate = "December 14, 2011";
...
/* The output file name */
  	string BitmapFile = "At 50 (cross section).bmp";
...
/* Reserve Field for Density and Current Information */
	DensCurr *QuantumField = new DensCurr[xSamples * ySamples];
...
/* Green Function - Open Channels */
	...
	/* One-Dimensional Green Function in Field Direction, and its Derivative */
		Green1DPrimeArray[Level] = polar(1.0, 2.0 * Aux * fabs(zPos));
		Green1DArray[Level] = -ii * Green1DPrimeArray[Level] / Aux;

	/* Apply Sign Correction, If Necessary */
		if (zPos < 0) Green1DPrimeArray[Level] *= (-1);
...
/* Green Function - Closed Channels */
	...
	if (Level == MaxLevel)
		cerr << "Warning:  Green function series did not converge. \n\n";
	else
		cerr << "Calculating 1D Green functions:  Success. \n\n";
...
/* Loop Through y Values */
	...
	/* Calculate Green Function, Derivative As Sums */
		...
		/* Apply Gaussian Prefactor */
			double Prefactor =  exp(-0.5 * Radial);
		...
		/* Store Density, Current Density Vector */
			...
			QuantumField[Ptr].CurrentAzimuthal = -4.0 * Aux * eps * eps * rho * norm(Sum);
		}
...
/* Loop Over z Values */
	...
	/* Loop Over x Values */
		...
		/* Paint Pixel (Note: Progress top-to-bottom) */
			PSet(ImageTemplate, BMPWidth, BMPHeight, j, ySamples - i - 1, Red, Green, Blue);
		}
...
/* Create Image Frame */
	for (i = 0; i < FrameWidth; ++i)
	{
		...
		for (j = 0; j < xSamples; ++j)
		{
			...
			PSet(ImageTemplate, BMPWidth, BMPHeight,
				j, ySamples - i - 1, FrameRed, FrameGreen, FrameBlue);
		}
	}
...
/* Create Ticks */
	...
	int yMinTick = (int)ceil(yMin / yTick);
	int yMaxTick = (int)floor(yMax / yTick);
	...
	for (int l = xMinTick; l <= xMaxTick; ++l)
	{
		...
		for (i = Lower; i <= Upper; ++i)
		{
			...
			for (j = ySamples - 1; j >= ySamples - TickLength; --j)
			{
				PSet(ImageTemplate, BMPWidth, BMPHeight,
					i , j, FrameRed, FrameGreen, FrameBlue);
			}
	...
	for (int l = yMinTick; l <= yMaxTick; ++l)
	{
	/* Position in y Space */
		double yCenter = l * yTick;

	/* Position in w Space */
		double Center = yCenter;

	/* Position on Canvas */
	 	Center = (Center - yMin) * (ySamples - 1) / (yMax - yMin);

		long Upper = long(Center + 0.5 * TickWidth + 0.5);
		if (Upper > ySamples - 1) Upper = ySamples - 1;
		long Lower = long(Center - 0.5 * TickWidth + 0.5);
		if (Lower < 0) Lower = 0;
